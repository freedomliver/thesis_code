function newdata = ReadSIF_single(filename)
% modified from the file sifreadexample.m in matlab toolbox Andor (installed by user)
% only retrieve the first frame in the SIF file (can be easily extended to all frames)
...
if (rc == 22002) % read successfully 
    ...
    if present
        ...
        if (num_frames > 0)
            ...
            [~,data]=atsif_getframe(signal,0,size); % retrieve a single frame(0-the first frame) in the SIF file
