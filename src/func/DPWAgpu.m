function [nonspinflip,spinflip] = DPWAgpu(q,atom,Ek,DPWAfile)
...
%
% nonspinflip: f(q) the direct scattering amplitude (cm); a gpu array
% spinflip: g(q) spin-flip scattering amplitude (cm); a gpu array
...
gpuq=gpuArray(q); % change q to a gpu array
gpuq=gpuq.*1e10; % (m^-1)
theta=2.*asin(gpuq./2./k);
input=cos(theta);
...
tmp=size(q);
P1=gpuArray(ones(tmp)); % legendre function P0(x)=1;
